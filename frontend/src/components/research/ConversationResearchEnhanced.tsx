...
const ConversationResearchEnhanced: React.FC = () => {
  ...
  const [researchSessionId, setResearchSessionId] = useState<string | null>(null);  // Separate session for research API
  ...
  const pollResearchStatus = useCallback(async (sid: string) => {
    try {
      // Use research endpoint for deep research mode streaming
      const response = await fetch(`${apiUrl}/api/v1/research/status-strands-real/${sid}`);
      if (!response.ok) return;
      
      const data = await response.json();
      console.log('Deep research status update:', data.status, 'Progress:', data.progress);
      ...
      // Build content from research data
      let displayContent = data.content || '';
      
      // Show research progress with steps and thoughts
      if (data.status === 'running') {
        // Show steps if available
        if (data.steps && data.steps.length > 0) {
          const activeSteps = data.steps.filter((step: any) => step.status === 'active' || step.status === 'completed');
          if (activeSteps.length > 0) {
            displayContent = `🔬 Deep Research Progress (${Math.round(data.progress || 0)}% complete)\n\n`;
            displayContent += activeSteps.map((step: any) => `• ${step.title}: ${step.description}`).join('\n');
            displayContent += '\n\n';
          }
        }
        
        // Add streaming thoughts if available
        if (data.thoughts && data.thoughts.length > 0) {
          const recentThoughts = data.thoughts.slice(-3);  // Show last 3 thoughts
          displayContent += '💭 Current Analysis:\n';
          displayContent += recentThoughts.map((thought: any) => `  ${thought.content || thought}`).join('\n');
        }
      }
  ...
  const pollForUpdates = useCallback(async (sid: string) => {
    try {
      ...
      // Update messages - just use what backend sends
      if (data.messages) {
        const backendMessages = [...data.messages];
        
        // Attach current thoughts/sources/screenshots to last assistant message
        if (backendMessages.length > 0) {
          for (let i = backendMessages.length - 1; i >= 0; i--) {
            if (backendMessages[i].role === 'assistant') {
              backendMessages[i].thoughts = data.thoughts || backendMessages[i].thoughts || [];
              backendMessages[i].screenshots = data.screenshots || backendMessages[i].screenshots || [];
              backendMessages[i].sources = data.sources || backendMessages[i].sources || [];
              backendMessages[i].status = data.status;
              backendMessages[i].progress = data.progress;
              backendMessages[i].mode = backendMessages[i].mode || mode;
              break;
            }
          }
        }
  ...
  const loadSession = async (sid: string) => {
    try {
      ...
      setSessionId(sid);
      setResearchSessionId(null);  // Clear research session when loading a conversation
  ...
  const startNewConversation = () => {
    ...
    setSessionId(null);
    setResearchSessionId(null);  // Also clear research session
    setMessages([]);
    setThoughts([]);
    setSources([]);
    setIsLoading(false);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.log(`Starting ${mode} mode for query:`, userMessage);
      console.log('Request body:', sessionId ? 'continue' : 'start', { mode, sessionId });
      
      let endpoint;
      let requestBody;
      
      if (mode === 'deep') {
        // For deep research mode, always start fresh research session
        endpoint = `${apiUrl}/api/v1/research/start-strands-real`;
        requestBody = {
          query: userMessage,
          session_id: null,  // Always start new for deep research
          require_approval: false
        };
      } else {
        // For conversation modes (fast/scholar), use conversation API
        // Continue existing conversation session if available and same mode
        const canContinue = sessionId && !researchSessionId; // Only continue if it was a conversation session
        endpoint = canContinue ? `${apiUrl}/api/v1/conversation/continue` : `${apiUrl}/api/v1/conversation/start`;
        requestBody = canContinue 
          ? { 
              message: userMessage,
      ...
      if (mode === 'deep') {
        // For deep research, mark this as a research session
        setResearchSessionId(newSessionId);
        setSessionId(newSessionId);  // Use same ID for URL
        navigate(`/conversation/${newSessionId}`, { replace: true });
      } else {
        // For conversation modes
        setSessionId(newSessionId);
        setResearchSessionId(null);  // Clear any research session
      ...
      if (!sessions.find(s => s.session_id === newSessionId)) {
        const newSession: ConversationSession = {
          ...
          timestamp: new Date().toISOString(),
          active: true,
          mode: mode  // Track the mode for this session
        };
      ...
      // Use appropriate polling based on mode
      if (mode === 'deep') {
        // Deep mode: Use research polling for streaming thoughts
        pollResearchStatus(newSessionId);
        
        // Continue polling research endpoint
        pollingIntervalRef.current = setInterval(() => {
          pollResearchStatus(newSessionId);
        }, 500);
      } else {
        // Fast/Scholar modes: Use conversation polling
        if (mode === 'fast') {
          setTimeout(() => pollForUpdates(newSessionId), 100);
        } else {
          pollForUpdates(newSessionId);
        }
        
        // Continue polling conversation endpoint
        pollingIntervalRef.current = setInterval(() => {
          pollForUpdates(newSessionId);
        }, 500);
      }
                ...
                <DropdownMenu.Root>
                  ...
                  <DropdownMenu.Content className="dropdown-content">
                    <DropdownMenu.Item onClick={() => {
                      if (mode !== 'fast' && messages.length > 0) {
                    ...
                    }}>⚡ Fast</DropdownMenu.Item>
                    <DropdownMenu.Item onClick={() => {
                      if (mode !== 'deep' && messages.length > 0) {
                        if (window.confirm('Switching modes will start a new chat. Continue?')) {
                          setMode('deep');
                          startNewConversation();
                        }
                    ...
                    }}>🔬 Deep Research</DropdownMenu.Item>
                    <DropdownMenu.Item onClick={() => {
                      if (mode !== 'scholar' && messages.length > 0) {
                        if (window.confirm('Switching modes will start a new chat. Continue?')) {
                          setMode('scholar');
                          startNewConversation();
                        }
